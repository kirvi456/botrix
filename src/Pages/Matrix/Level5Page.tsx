...
export const Level5Page = () => {
    ...
    const getInformation = async () => {
        try {
            ...
            const tokenId = await LVL5Contract.methods.players(currentAccount).call();
            setCountInfo((prev) => ({ ...prev, tokenId }));

            if (tokenId == 0) return;

            const information = await LVL5Contract.methods.users(Number(tokenId) - 1).call();
            ...
            const referred = await LVL5Contract.methods.getPlayerReferred(currentAccount).call();
            ...
            const connections = await LVL5Contract.methods
            ...
            const treeSTR = (await LVL5Contract.methods
    ...
    useEffect(() => {
        if (LVL5Contract) getInformation();
    }, [LVL5Contract]);
